...
![19](https://user-images.githubusercontent.com/36281158/91641634-56e86280-ea43-11ea-9177-08f18fd13fc9.PNG)


PS: Lots of training is done to find the parameters which will give the decent summary. Initially the model was not performing well may not removing the noise properly, varying batch size. Also i find out that it is okay that loss should be as much as minimum as possible but it dosent gurantee that min loss will result in good summary so another metric is needed to judge this (Bleu Score) 
...
3. Model Strucuture change like adding more LSTM layers or dense layers

4. Trying attention mehcanism as it is good if the documents for training are large 
